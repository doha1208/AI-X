...
교통사고 다발지역은 여기 넣지 않는다 — 동 단위 accident_dist_m(safety_score.py)이 이미 그
신호를 담고 있고, safe_route.py의 _edge_score가 동 점수(zone_score)와 이 구간 밀도 점수를
같이 더하는 구조라 여기서도 넣으면 같은 사고다발지역 데이터가 두 번(동 점수 + 구간 밀도)
반영돼 실제 가중치보다 영향력이 커진다.
...
CCTV_RADIUS_M = 60.0
LIGHT_RADIUS_M = 40.0
# 반경 안에 이 개수 이상이면 만점이다 — 더 많다고 계속 안전해지지는 않는다.
CCTV_SATURATION = 1
LIGHT_SATURATION = 2
